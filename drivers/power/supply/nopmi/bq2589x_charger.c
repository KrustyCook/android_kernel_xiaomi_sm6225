...
// modify by HTH-209427/HTH-209841 at 2022/05/12 end
	...
	if (ret < 0) {
		bq_dbg(PR_OEM, "%s: Failed to set charge voltage:c%d\n", __func__, ret);
		return ret;
	}
...
extern int get_prop_battery_charging_enabled(struct votable *usb_icl_votable, union power_supply_propval *val);
...
static int bq2589x_wall_set_property(struct power_supply *psy, enum power_supply_property psp, const union power_supply_propval *val)
{
	int ret = 0;
	struct bq2589x *bq = power_supply_get_drvdata(psy);
...
static int bq2589x_wall_prop_is_writeable(struct power_supply *psy, enum power_supply_property psp)
{
	switch (psp) {
	case POWER_SUPPLY_PROP_CHARGE_TYPE:
	case POWER_SUPPLY_PROP_ONLINE:
...
static void bq2589x_usb_changed_workfunc(struct work_struct *work)
{
	...
	if (notify_count < NOTIFY_COUNT_MAX) {
		if (bq->usb_psy) {
			...
			if (chg_type == POWER_SUPPLY_TYPE_USB_PD
...
static void bq2589x_charger_irq_workfunc(struct work_struct *work)
{
	...
	if (bq->vbus_type == BQ2589X_VBUS_USB_CDP
		|| bq->vbus_type == BQ2589X_VBUS_USB_SDP)
		bq2589x_usb_switch(bq, false);

	if ((bq->vbus_type == BQ2589X_VBUS_NONSTAND
...
static int bq2589x_set_cc_orientation(struct bq2589x *bq, int cc_orientation)
{
	...
	ret = power_supply_set_property(bq->usb_psy, POWER_SUPPLY_PROP_TYPEC_CC_ORIENTATION, &propval);
	if (ret < 0)
...
static int bq2589x_set_typec_mode(struct bq2589x *bq, enum power_supply_typec_mode typec_mode)
{
	...
	ret = power_supply_set_property(bq->usb_psy, POWER_SUPPLY_PROP_TYPEC_MODE, &propval);
	if (ret < 0)
...
static int fcc_vote_callback(struct votable *votable, void *data, int fcc_ua, const char *client)
{
	struct bq2589x *bq = data;
	int rc;
...
static int chg_dis_vote_callback(struct votable *votable, void *data, int disable, const char *client)
{
	struct bq2589x *bq = data;
	int rc;
	...
	if (rc < 0) {
		bq_dbg(PR_OEM, "failed to disable: %d\n", disable);
		return rc;
	}
...
static int fv_vote_callback(struct votable *votable, void *data, int fv_mv, const char *client)
{
	struct bq2589x *bq = data;
	int rc;
...
static int usb_icl_vote_callback(struct votable *votable, void *data, int icl_ma, const char *client)
{
	int rc;
...
/*
static int chgctrl_vote_callback(struct votable *votable, void *data, int disable, const char *client)
{
	struct bq2589x *bq = data;
	int rc;
