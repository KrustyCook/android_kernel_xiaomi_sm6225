...
static void start_nopmi_chg_workfunc(void)
{
	...
	if (g_nopmi_chg) {
		...
		schedule_delayed_work(&g_nopmi_chg->cvstep_monitor_work, msecs_to_jiffies(NOPMI_CHG_CV_STEP_MONITOR_WORKFUNC_GAP));
	}
}
...
static int nopmi_chg_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		nopmi_chg = devm_kzalloc(&pdev->dev, sizeof(struct nopmi_chg),
			GFP_KERNEL);
		if (!nopmi_chg) {
